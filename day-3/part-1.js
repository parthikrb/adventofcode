...
const getMatchingState = (firstHalf, secondHalf) => {
...
const getCharacterCode = (character) => {
  ...
  const characterCode = upperCaseAlphabet.includes(character)
    ? characterIndex + 27
    : characterIndex + 1;
...
const rucksackPriority = (input) => {
  ...
  splitInput.map((rucksack) => {
    const firstHalf = rucksack.slice(0, rucksack.length / 2);
    const secondHalf = rucksack.slice(rucksack.length / 2);
    const matchingState = getMatchingState(firstHalf, secondHalf);
    characterPriorities += getCharacterCode(matchingState);
  });
